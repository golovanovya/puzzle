...
for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
        ...
        tile.element = drawTile(tileSize, tile);
        tiles.push(tile);
    }
}
function hasTop(row) {
...
function drawTile(size, tile) {
    const gap = 50;
    const element = document.createElement('ul');
    element.className = 'puzzle__tile draggable';
    element.style = `top:${tile.y * size + tile.y * gap}px;left:${tile.x * size + tile.x * gap}px`;
    let tileContent = '';
    ...
    element.addEventListener('mouseover', mouseOverTile);
    return element;
}
...
document.addEventListener('mousedown', function (event) {
    ...
    function moveAt(clientX, clientY) {
        ...
        dragElement.dispatchEvent(new Event('dragging'));
    }
